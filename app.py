...
def index():
    ...
    total_volume = db.execute('SELECT COALESCE(SUM(weight_kg * reps), 0) FROM entries').fetchone()[0] or 0
...
def stats():
    ...
    records = db.execute('''
        SELECT e.exercise, MAX(e.weight_kg) as max_weight, s.date as record_date
        FROM entries e
    ...
    try:
        ...
        for ex in data.get('entries', []):
            ...
            for i, s in enumerate(ex.get('sets', []), 1):
                ...
                if reps or weight:
                    db.execute(
                        'INSERT INTO entries (session_id, exercise, set_number, reps, weight_kg) VALUES (?,?,?,?,?)',
...
def get_session_by_date(date_str):
    ...
    for e in entries:
        ...
        if name not in ex_map:
            ex_map[name] = []
        ex_map[name].append({'set': e['set_number'], 'reps': e['reps'], 'weight': e['weight_kg']})
...
def get_progress(exercise):
    ...
    data = db.execute('''
        SELECT s.date,
               ...
               ROUND(SUM(e.weight_kg * e.reps), 1) as volume,
